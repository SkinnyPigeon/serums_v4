...
jwt_value='eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..zIHdEG0nbjo_dDrJR6xz4fwKReIIkdtei3FV0X_l2Ls'


default_jwt = f"Bearer {jwt_value}"
...
dv_reply_fields = api.model('Data Vault Successfully Created and Encrypted', {
    'key': fields.String(required=True, description='The encrypted Fernet key', example=""),
...
class SPHR(Resource):
    @api.expect(sphr_parser, request_fields_no_pk)
    def post(self):
...
class DV(Resource):
    @api.expect(dv_parser, dv_request_fields_no_pk)
    def post(self):
...
class DVEncrypted(Resource):
    ...
    @api.marshal_with(dv_reply_fields)
    def post(self):
